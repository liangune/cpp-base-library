...
#ifdef _MSC_VER
#define ZLOG_CONF_BACKUP_ROTATE_LOCK_FILE "c:/zlogtmp/zlog.lock"
#else
...
zlog_conf_t *zlog_conf_new(const char *confpath)
{
	...
	if (confpath && confpath[0] != '\0' && file_exists (confpath)) {
		nwrite = snprintfs(a_conf->file, sizeof(a_conf->file), "%s", confpath);
		has_conf_file = 1;
	} else if (getenv("ZLOG_CONF_PATH") != NULL) {
		nwrite = snprintfs(a_conf->file, sizeof(a_conf->file), "%s", getenv("ZLOG_CONF_PATH"));
		has_conf_file = 1;
	} else {
